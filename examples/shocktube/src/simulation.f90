...
contains
   ...
   subroutine simulation_init
      ...
      create_and_initialize_flow_solver: block
         ...
         fs%Lrho = 1.0_WP; fs%LrhoE = 1.0_WP; fs%Vi = 0.0_WP; fs%Wi = 0.0_WP
         select case (trim(adjustl(shtb_type)))
         case ('sod')
            v0 = 0.0_WP ! Velocity at inflow
            do i=fs%cfg%imino_,fs%cfg%imaxo_
               if (fs%cfg%xm(i).le.0.5_WP) then
                  fs%Grho (i,:,:) = 1.0_WP
                  fs%Ui   (i,:,:) = 0.0_WP
                  fs%GrhoE(i,:,:) = matmod%EOS_energy(1.0_WP,1.0_WP,0.0_WP,0.0_WP,0.0_WP,'gas')
               else
                  fs%Grho (i,:,:) = 0.125_WP
                  fs%Ui   (i,:,:) = 0.0_WP
                  fs%GrhoE(i,:,:) = matmod%EOS_energy(0.1_WP,0.125_WP,0.0_WP,0.0_WP,0.0_WP,'gas')
               end if
            end do
         case('shu-osher')
            v0 = 2.629369_WP ! Velocity at inflow
            do i=fs%cfg%imino_,fs%cfg%imaxo_
               if (fs%cfg%xm(i).le.-4.0_WP) then
                  fs%Grho (i,:,:) = 3.857143_WP
                  fs%Ui   (i,:,:) = v0
                  fs%GrhoE(i,:,:) = matmod%EOS_energy(10.33333_WP,3.857143_WP,v0,0.0_WP,0.0_WP,'gas')
               else
                  dens = 1.0_WP+0.2_WP*sin(5.0_WP*fs%cfg%xm(i))
                  fs%Grho (i,:,:) = dens
                  fs%Ui   (i,:,:) = 0.0_WP
      ...
      create_ensight: block
         ...
         call ens_out%add_scalar('InternalEnergy',fs%RHOSS2)
         ! Output to ensight
   ...
   subroutine simulation_final
      ...
      real(WP) :: buf1
      real(WP), dimension(:), allocatable :: dens, vel, pres, iegy
      character(len=str_long) :: line, header

      ! Output profiles here for easy postprocessing
      ! Allocate and zero profile arrays
      allocate(dens(fs%cfg%nx),vel(fs%cfg%nx),pres(fs%cfg%nx),iegy(fs%cfg%nx))
      dens = 0.0_WP; vel = 0.0_WP; pres = 0.0_WP; iegy = 0.0_WP

      j = fs%cfg%jmin_
      k = fs%cfg%kmin_
      ! Put local info into array (assumes 1D simulation)
      dens(fs%cfg%imin_:fs%cfg%imax_)=fs%RHO(fs%cfg%imin_:fs%cfg%imax_,j,k)
      vel(fs%cfg%imin_:fs%cfg%imax_)=fs%Ui(fs%cfg%imin_:fs%cfg%imax_,j,k)
      pres(fs%cfg%imin_:fs%cfg%imax_)=fs%P(fs%cfg%imin_:fs%cfg%imax_,j,k)
      do i=fs%cfg%imin_,fs%cfg%imax_
         iegy(i) = matmod%EOS_energy(matmod%EOS_gas(i,j,k,'p'),dens(i),0.0_WP,0.0_WP,0.0_WP,'gas')
      end do

      ! Create final profile array using info from every proc
      do n=1,fs%cfg%nx
         ! Sum density, velocity, pressure, internal energy
         call MPI_ALLREDUCE(dens(n),buf1,1,MPI_REAL_WP,MPI_SUM,fs%cfg%comm,ierr); dens(n) = buf1;
         call MPI_ALLREDUCE(vel(n) ,buf1,1,MPI_REAL_WP,MPI_SUM,fs%cfg%comm,ierr); vel(n)  = buf1;
         call MPI_ALLREDUCE(pres(n),buf1,1,MPI_REAL_WP,MPI_SUM,fs%cfg%comm,ierr); pres(n) = buf1;
         call MPI_ALLREDUCE(iegy(n),buf1,1,MPI_REAL_WP,MPI_SUM,fs%cfg%comm,ierr); iegy(n) = buf1;
      end do
      ! Write profile to file
      if (fs%cfg%amRoot) then
         ! Open file
         open(newunit=iunit,file=trim(adjustl(shtb_type))//'.txt',form="formatted",iostat=ierr,status="REPLACE")
         ! Write header
         write (header(1+0*col_len:),aformat) 'xm'
         write (header(1+1*col_len:),aformat) 'RHO'
         write (header(1+2*col_len:),aformat) 'U'
         write (header(1+3*col_len:),aformat) 'P'
         write (header(1+4*col_len:),aformat) 'IE'
         write(iunit,'(a)') trim(header)
         ! Write data
         do n=1,fs%cfg%nx
            ! Create the line to dump
            write (line(1+0*col_len:),rformat) fs%cfg%xm(n)
            write (line(1+1*col_len:),rformat) dens(n)
            write (line(1+2*col_len:),rformat) vel(n)
            write (line(1+3*col_len:),rformat) pres(n)
            write (line(1+4*col_len:),rformat) iegy(n)
            ! Dump the line
